...
class EstradSecuritySoldier(BaseDialog):
    def __init__(self, bot_game, estrad):
        super().__init__(bot_game, ESTRAD_SECURITY_SOLDIER, 'Солдат Межгалактической Республики',
    ...
    def handle(self, message):
        if message.text == '[🗣Харизма 4] Меня прислало высшее руководство передать срочное послание вашему начальнику.':
            if self.bot_game.players[message.chat.id].charisma >= 4:
            ...
            if self.bot_game.players[message.chat.id].money >= 250:
                self.bot_game.players[message.chat.id].money -= 250
                self.say(message, 'Хорошо, проходи.')
            else:
                self.say(message, 'У тебя и денег то таких нет.')
        elif message.text == 'Мне уже пора':
            self.bot_game.bot.send_message(message.chat.id, 'До встречи.')
